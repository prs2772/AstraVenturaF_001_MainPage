...
docker build -t <login-server>/avf-api:v1.0.0 .

# 3. Sube la imagen a la nube
docker push <login-server>/avf-api:v1.0.0
```
...
# 2. Construye la imagen etiquetada para Azure
docker build -t astraventuraauth-e2d6h4ajd7caejad.azurecr.io/avf-api:v1.0.0 .

# 3. Sube la imagen a la nube
docker push astraventuraauth-e2d6h4ajd7caejad.azurecr.io/avf-api:v1.0.0
```
